...
var (
	red         = RGBAToNRGBA(colornames.Red)
	black       = RGBAToNRGBA(colornames.Black)
...
func run(t *testing.T, f func(o *op.Ops), c func(r result)) {
	...
	if !verifyRef(t, img, 0) {
		name := fmt.Sprintf("%s-bad.png", t.Name())
	...
	case *image.NRGBA:
		...
		for x := bnd.Min.X; x < bnd.Max.X; x++ {
			for y := bnd.Min.Y; y < bnd.Max.Y; y++ {
				ref.SetRGBA(x, y, NRGBAToRGBA(r.NRGBAAt(x, y)))
			}
		}
	default:
...
func colorsClose(c1, c2 color.RGBA) bool {
	const delta = 0.05 // magic value obtained from experimentation.
...
func saveImage(t testing.TB, file string, img *image.RGBA) {
	...
	for x := bnd.Min.X; x < bnd.Max.X; x++ {
		for y := bnd.Min.Y; y < bnd.Max.Y; y++ {
			nrgba.SetNRGBA(x, y, RGBAToNRGBA(img.RGBAAt(x, y)))
		}
	}
